...
_Audited: 2026-04-28 13:48 by TBA_
_Status: ✅ READY FOR FIRST CUSTOMER. Price ID mapping remains fixed (incl. SCALE), and fresh production auth → checkout passed again. Current proof: `/tmp/councilnow-revenue-proof-reminder-1777377471.json` — health/register/login/me/subscription/portal plus starter/pro/scale/enterprise Stripe checkout sessions all `200`, each checkout URL `HEAD 200`._
...
| Stripe keys | ✅ LIVE | Fresh 13:48 production proof created/resolved Stripe customer `cus_UQ0HHf0AlzlZ82`; expired-key failure remains resolved. |
| Stripe checkout API | ✅ ALL TIERS VERIFIED | Fresh 13:48 production proof: `/api/billing/checkout` returned live Stripe sessions for starter/pro/scale/enterprise and each Stripe checkout URL returned `HEAD 200`. |
| Stripe prices configured | ✅ | All 4 tiers: Starter (€29), Pro (€79), Scale (€199), Enterprise (€499) EUR prices |
| Customer portal | ✅ LIVE | Fresh 13:48 production proof: `/api/billing/portal` returned `200` for Stripe customer `cus_UQ0HHf0AlzlZ82`. |
...
2. **Stripe checkout with real user:** ✅ VERIFIED on production. Authenticated token flow passes for `/api/billing/checkout` (all tiers), `/api/billing/portal`, and `/api/billing/subscription`.
   - Fresh 13:48 recheck PASS with new production account `oc-reminder-1777377471-77326@example.com`: `/health` ok, register/login/me/subscription/portal all returned `200/201`, linked Stripe customer `cus_UQ0HHf0AlzlZ82`, and starter/pro/scale/enterprise checkout sessions plus Stripe checkout URL `HEAD` probes returned `200`. Proof: `/tmp/councilnow-revenue-proof-reminder-1777377471.json`.
   - Fresh 05:48 recheck PASS with new production account `oc-reminder-1777348351-97824@example.com`: `/health` ok, register/login/me/subscription/portal all returned `200/201`, linked Stripe customer `cus_UPsSrVRBVCxPqc`, and starter/pro/scale/enterprise checkout sessions plus Stripe checkout URL `HEAD` probes returned `200`. Proof: `/tmp/councilnow-revenue-proof-reminder-1777348351.json`.
   - Fresh 03:48 recheck PASS with new production account `oc-reminder-1777341182-14343@example.com`: `/health` ok, register/login/me/subscription/portal all returned `200/201`, linked Stripe customer `cus_UPqWtjpjnoZ0TL`, and starter/pro/scale/enterprise checkout sessions plus Stripe checkout URL `HEAD` probes returned `200`. Proof: `/tmp/councilnow-revenue-proof-reminder-1777341182.json`.
   ...
   - Fresh 21:10 post-deploy recheck PASS with new production account `oc-deploy-1777230648-76241@example.com`: `/health` ok, register/login/me passed, `/api/billing/subscription` and `/api/billing/portal` returned `200`, and starter/pro/scale/enterprise checkout sessions plus Stripe checkout URL `HEAD` probes returned `200`. Proof: `/tmp/councilnow-revenue-proof-after-deploy-1777230648.json`.
   - Fresh 21:14 final post-deploy recheck PASS with new production account `oc-final-1777230877-56298@example.com`: `/health` ok, register/login/me passed, subscription and portal returned `200`, and starter/pro/scale/enterprise checkout API plus Stripe checkout URL `HEAD` probes all returned `200`. Proof: `/tmp/councilnow-revenue-proof-final-1777230877.json`.
   - Fresh 22:36 reminder recheck PASS with new production account `oc-reminder-1777235801-79840@example.com`: `/health` ok, register/login/me passed, subscription and portal returned `200`, and starter/pro/scale/enterprise checkout API plus Stripe checkout URL `HEAD` probes all returned `200`. Proof: `/tmp/councilnow-revenue-proof-reminder-1777235801.json`.
